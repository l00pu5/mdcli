...
export async function treeCommand(filePath) {
  ...
  if (headings.length === 0) {
    console.log(boxen(
      `${chalk.cyan.bold('📄 ' + titleHeader)}\n\n${chalk.yellow('⚠️ No MD headings (H1-H6) found in this file')}`,
  ...
  const stats = chalk.gray(
    `📊 Headings: ${headings.length} | Layers: ${[...new Set(headings.map(h => 'H' + h.level))].sort().join(', ')}`
  );
  ...
  console.log(
    boxen(
      `${chalk.cyan.bold('📄 ' + titleHeader)}\n\n${treeOutput}\n${stats}`,
      {
        ...
        borderColor: 'cyan',
        borderStyle: 'round',
        title: chalk.bold(' Heading structure '),
        titleAlignment: 'left'
      }
    )
  );
